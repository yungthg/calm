...
public class MazeSolverIDDFS {
    ...
    public static boolean solveMazeUsingIDDFS(int x, int y, boolean[][] visited, List<int[]> path) {
        int maxDepth = rows * cols; // Maximum depth limit
        for (int depth = 0; depth <= 8; depth++) {
            visited = new boolean[rows][cols]; // Reset visited array
            if (solveMazeUsingDLDFS(x, y, visited, path, depth)) {
                return true;
            }
        }
        return false; // No solution found within the maximum depth
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (solveMazeUsingIDDFS(0, 0, new boolean[rows][cols], path)) {
                System.out.println("IDDFS: Path found!");
                System.out.println("IDDFS Path:");
                Collections.reverse(path);
                printSolutionPath(path);
            } else {
                System.out.println("IDDFS: No path found.");
            }
